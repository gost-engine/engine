...
int gost94_nid_by_params(DSA *p)
{
    ...
    BIGNUM *q = BN_new();
    for (gost_params = R3410_paramset; gost_params->nid != NID_undef;
         gost_params++) {
...
static ASN1_STRING *encode_gost_algor_params(const EVP_PKEY *key)
{
    ...
    case NID_id_GostR3410_94:
        pkey_param_nid = (int)gost94_nid_by_params(key_ptr);
        gkp->hash_params = OBJ_nid2obj(NID_id_GostR3411_94_CryptoProParamSet);
        break;
    }
...
static int gost_decode_nid_params(EVP_PKEY *pkey, int pkey_nid, int param_nid)
{
    ...
    case NID_id_GostR3410_94:
        ...
        return fill_GOST94_params(key_ptr, param_nid);
    }
...
static int priv_encode_gost(PKCS8_PRIV_KEY_INFO *p8, const EVP_PKEY *pk)
{
    ...
    M_ASN1_OCTET_STRING_set(octet, buf, key_len);
...
static int pub_cmp_gost94(const EVP_PKEY *a, const EVP_PKEY *b)
{
    ...
    if (!da->pub_key || !db->pub_key)
        return 0;

    return (BN_cmp(da->pub_key, db->pub_key) == 0) ? 1 : 0;
}
                               ...
                               int derlen)
{
    ...
    return gost_decode_nid_params(pkey, NID_id_GostR3410_94, nid);
}
